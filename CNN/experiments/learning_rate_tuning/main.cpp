...
//
// Scientific question: How do different learning rate magnitudes (1e-5, 1e-3, 1e-1)
// and dynamic scheduling policies (Constant, Step Decay, Cosine Annealing, and
// Warmup + Cosine Decay) affect the convergence speed, stability, and generalization
// of the CNN surrogate model when trained with the winning Adam optimizer under the
// SIMM physics-informed loss?
//
...
//   model selection by refitting the winning schedule configuration on the
//   entire training set.
//
...
public:
    ...
    FoldMetrics run(const TrialConfig& config,
        ...
        record.scheduler_name = config.scheduler.description();
...
void write_fold_records_csv(const std::string& path,
    ...
    csv << std::setprecision(10)
        << "candidate,scheduler,fold,train_mse,validation_mse,training_objective,epochs\n";
    for (const auto& r : records) {
        csv << '"' << r.candidate << "\",\"" << r.scheduler_name << "\","
...
void write_cv_summary_csv(const std::string& path,
                          const std::vector<CandidateResult>& ranked_candidates) {
    std::ofstream csv(path);
    if (!csv) {
        throw std::runtime_error("Cannot open " + path + " for writing.");
    }
    csv << std::setprecision(10)
        << "candidate,mean_val_mse,std_val_mse,min_val_mse,max_val_mse,mean_train_mse,fold_count\n";
    for (const auto& c : ranked_candidates) {
        double min_val = std::numeric_limits<double>::infinity();
        double max_val = -std::numeric_limits<double>::infinity();
        double sum_train = 0.0;
        for (const auto& f : c.folds) {
            min_val = std::min(min_val, f.validation_mse);
            max_val = std::max(max_val, f.validation_mse);
            sum_train += f.training_objective;
        }
        const double mean_train = c.folds.empty() ? 0.0 : sum_train / static_cast<double>(c.folds.size());
        csv << '"' << c.config.name << "\","
            << c.mean_validation_mse << ','
            << c.validation_stddev << ','
            << min_val << ','
            << max_val << ','
            << mean_train << ','
...
struct ProgramOptions {
    std::string mode = "tune"; // "tune", "smoke"
    ...
    std::string results_dir = "results/cross_validation/learning_rate_tuning";
...
void print_help() {
    std::cout
        << "Learning Rate Schedule Tuning Experiment for CNN Wing Ice Model\n\n"
        << "Usage: learning_rate_tuning [options]\n\n"
        << "Options:\n"
        << "  --mode <tune|smoke>         Experiment mode (default: tune)\n"
        << "                                tune:  evaluates all 11 LR schedule configurations\n"
...
TrialConfig make_base_config(const ProgramOptions& options) {
    ...
    if (options.diagnostics) {
        ...
        training_cfg.diagnostics.experiment_name = "learning_rate_tuning";
    ...
    return TrialConfig{
        ...
        Recipes::adam(1e-3f), // Base winning Adam optimizer
...
ParameterGrid build_tuning_grid(const ProgramOptions& options) {
    TrialConfig base = make_base_config(options);
    ParameterGrid grid(base);

    // 11 Candidates evaluating learning rate magnitude & schedule dynamics:
    // 1. const_1e-5: Constant 1e-5
    // 2. const_1e-3: Constant 1e-3
    // 3. const_1e-1: Constant 1e-1
    // 4. step_s30_1e-1: Step decay (lr0=1e-1, step=30, gamma=0.1)
    // 5. step_s30_1e-3: Step decay (lr0=1e-3, step=30, gamma=0.1)
    // 6. step_s30_1e-5: Step decay (lr0=1e-5, step=30, gamma=0.1)
    // 7. step_s15_1e-1: Step decay (lr0=1e-1, step=15, gamma=0.5)
    // 8. step_s15_1e-3: Step decay (lr0=1e-3, step=15, gamma=0.5)
    // 9. step_s15_1e-5: Step decay (lr0=1e-5, step=15, gamma=0.5)
    // 10. cosine_1e-1: Cosine Annealing (min=1e-5, max=1e-1)
    // 11. warmup_cosine_1e-1: Warmup + Cosine (start=1e-5, peak=1e-1, min=1e-5, warmup=5)
    struct SchedConfig {
        std::string name;
        float base_adam_lr;
    ...
    std::vector<NamedChoice<SchedConfig>> choices = {
        {"const_1e-5", SchedConfig{"const_1e-5", 1e-5f, Recipes::constant_lr(1e-5f)}},
        {"const_1e-3", SchedConfig{"const_1e-3", 1e-3f, Recipes::constant_lr(1e-3f)}},
        {"const_1e-1", SchedConfig{"const_1e-1", 1e-1f, Recipes::constant_lr(1e-1f)}},
        {"step_s30_1e-1", SchedConfig{"step_s30_1e-1", 1e-1f, Recipes::step_lr(1e-1f, 30, 0.1f)}},
        {"step_s30_1e-3", SchedConfig{"step_s30_1e-3", 1e-3f, Recipes::step_lr(1e-3f, 30, 0.1f)}},
        {"step_s30_1e-5", SchedConfig{"step_s30_1e-5", 1e-5f, Recipes::step_lr(1e-5f, 30, 0.1f)}},
        {"step_s15_1e-1", SchedConfig{"step_s15_1e-1", 1e-1f, Recipes::step_lr(1e-1f, 15, 0.5f)}},
        {"step_s15_1e-3", SchedConfig{"step_s15_1e-3", 1e-3f, Recipes::step_lr(1e-3f, 15, 0.5f)}},
        {"step_s15_1e-5", SchedConfig{"step_s15_1e-5", 1e-5f, Recipes::step_lr(1e-5f, 15, 0.5f)}},
        {"cosine_1e-1", SchedConfig{"cosine_1e-1", 1e-1f, Recipes::cosine_lr(1e-5f, 1e-1f)}},
        {"warmup_cosine_1e-1", SchedConfig{"warmup_cosine_1e-1", 1e-5f, Recipes::warmup_cosine_lr(1e-5f, 1e-1f, 1e-5f, 5)}}
    };

    grid.add_choice<SchedConfig>(
        "schedule", choices,
        [](TrialConfig& trial, const SchedConfig& cfg) {
            trial.optimizer = Recipes::adam(cfg.base_adam_lr);
            trial.scheduler = cfg.scheduler;
        });
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        if (rank == 0) {
            std::cout << "========================================================\n"
                      << "    CNN LEARNING RATE SCHEDULE TUNING EXPERIMENT        \n"
                      << "========================================================\n"
                      << "Mode:            " << options.mode << "\n"
                      << "Folds:           " << options.folds << "\n"
        ...
        ParameterGrid grid = build_tuning_grid(options);
        ...
        if (rank == 0) {
            std::cout << "Starting Cross-Validation across "
                      << grid.candidates().size() << " learning rate schedule candidates...\n\n";
        }
        ...
        if (rank == 0) {
            ...
            const std::string cv_summary_csv_path = options.results_dir + "/cv_summary.csv";
            ...
            write_cv_summary_csv(cv_summary_csv_path, ranked_candidates);

            std::cout << "\n========================================================\n"
                      << "                 CROSS-VALIDATION RESULTS               \n"
                      << "========================================================\n";
            std::cout << std::left << std::setw(40) << "Candidate / Schedule"
            ...
            for (const auto& candidate : result.candidates) {
                std::cout << std::left << std::setw(40) << candidate.config.name;
                if (candidate.success) {
            ...
            std::cout << "\n--------------------------------------------------------\n"
                      << "RANKED SCHEDULES (BEST TO WORST):\n"
                      << "--------------------------------------------------------\n";
            for (size_t i = 0; i < ranked_candidates.size(); ++i) {
                const auto& c = ranked_candidates[i];
                std::cout << "  #" << std::setw(2) << (i + 1) << " " << std::left << std::setw(38) << c.config.name
                          << " | Val MSE: " << c.mean_validation_mse
                          << " +/- " << c.validation_stddev << "\n";
            }

            std::cout << "\n--------------------------------------------------------\n"
                      << "WINNING SCHEDULE: " << best.config.name << "\n"
                      ...
                      << "Cross-validation summary saved to: " << cv_summary_csv_path << "\n"
                      << "========================================================\n\n";

            std::cout << "Refitting winning schedule on full training dataset...\n";
        }
        ...
        if (rank == 0) {
            std::cout << "\n========================================================\n"
                      ...
                      << "========================================================\n"
                      << "Selected Schedule:         " << best.config.name << "\n"
            ...
            if (summary) {
                summary << "LEARNING RATE SCHEDULE TUNING EXPERIMENT SUMMARY\n"
                        << "=================================================\n"
                        << "Mode:                        " << options.mode << "\n"
                        << "Folds:                       " << options.folds << "\n"
                        << "Epochs:                      " << options.epochs << "\n"
        ...
        std::cerr << "Learning rate tuning failed on rank " << rank << ": "
                  << error.what() << '\n';
        if (world_size > 1) {
