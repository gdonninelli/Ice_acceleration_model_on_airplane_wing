...
from the two 200-epoch seed-42 runs with global batch size 64 and writes two
report figures showing that balancing the batches removed the recurrent
physical-MSE excursions.

Trial 1 (slurm-55367279_trial_1): original range-based batching, 24 batches
of 64 plus a final 6-sample tail.
Trial 2 (slurm-55373639_trial_2): balanced batching, 17 batches of 62 and
8 batches of 61, same 25 optimizer steps per epoch.
"""
...
TRIAL1 = Path("results/ordinary-training/slurm-55367279_trial_1")
TRIAL2 = Path("results/ordinary-training/slurm-55373639_trial_2")
...
def read_mse(path: Path) -> tuple[list[int], list[float], list[float]]:
    ...
    with path.open("r", newline="", encoding="utf-8") as handle:
        for row in csv.DictReader(handle):
            ...
            val.append(v)
    if len(epochs) != 200:
        raise ValueError(f"Expected 200 epochs in {path}, found {len(epochs)}")
...
def read_update_layer(path: Path, layer_index: int) -> tuple[list[int], list[float]]:
    ...
    if len(epochs) != 200:
        raise ValueError(f"Expected 200 update rows in {path}, found {len(epochs)}")
    return epochs, ratio
...
def main() -> None:
    ...
    layers1, rms1, _ = read_gradient_depth(args.trial1 / "gradient_norms.csv", 93)
    layers2, rms2, _ = read_gradient_depth(args.trial2 / "gradient_norms.csv", 93)
    # Stable trainable layer 4 is the first dense layer DenseLayer(7202, 128).
    ...
    if ue1 != ue2:
        raise ValueError("Update-ratio epoch grids differ between trials")
    ...
    plot_gradient_update(layers1, rms1, layers2, rms2, ue1, u1, u2, diag_path)
