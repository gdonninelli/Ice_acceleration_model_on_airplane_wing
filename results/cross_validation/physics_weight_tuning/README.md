# Physics-Weight Tuning Result Provenance

The current experiment driver in
`CNN/experiments/physics_weight_tuning/main.cpp` uses the topology selected by
the layer-architecture search:
...
`sweep_physics.csv` and to the diagnostic files under `sweep/`.
...
the physics weight on the winning topology. The source revision recorded by
the old diagnostics is `unknown`.
