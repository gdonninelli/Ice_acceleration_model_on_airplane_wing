...
%This differs from the configuration actually used in production: the ordinary model built by \texttt{make\_single\_trial} in \texttt{CNN/main.cpp} retains the $5\times5$ convolution with eight channels and stride five, together with LeakyReLU at $\alpha=0.05$, but adopts the smaller dense head $(128,64,1)$ and the source defaults, namely Adam at learning rate $10^{-3}$, physics weight $0.10$, no dropout, no L1 or L2 penalty, gradient clipping at 1, batch size 257, and seed 42. The two configurations therefore differ in head capacity, learning rate, physics weight, and batch size. We retain the topology winner as the lowest observed candidate of this grid; the figures reported for the final long training run in Section~\ref{sec:04_final_training} describe the recorded production configuration and must not be read as a test score for the large topology winner.
...
In contrast, the bottom two panels show the detailed diagnostics for the winning LeakyRELU candidate. The gradients are well-behaved across the network's depth. After an initial transient at epoch 1, the gradients at epoch 100 are small and stable, indicating smooth convergence without any signs of vanishing gradients. In addition to this, the post-activation variance decreases gracefully as information flows deeper into the network. Crucially, the variance stabilizes and never collapses to zero, confirming that the network is actively learning and propagating signals effectively through all layers for the entire duration of training.\\
...
The first two trials form a controlled comparison of the batch construction because every other recorded setting is identical. Both \texttt{slurm-55367279\_trial\_1} and \texttt{slurm-55373639\_trial\_2} completed 200 epochs with seed 42, Adam at configured and effective learning rate $10^{-3}$, physics weight $0.10$, no dropout and no L1 or L2 penalty, element-wise gradient clipping at 1, global batch size 64, and 64 MPI ranks on the same 1542-sample ordinary training split with 171 validation samples. Both histories report 25 batches per epoch in \texttt{epoch\_metrics.csv}, so each run performed 5000 optimizer steps, and both used the production small head $(128,64,1)$ with the $5\times5$ convolution described below rather than the large topology selected in the separate topology search. The only recorded difference is the source revision and hence the batching code: the first trial used the original range-based batching with 24 batches of 64 samples and a final batch of only 6 samples, while the second trial used the balanced construction in Equation~\eqref{eq:04_final_batch_partition} with 17 batches of 62 and 8 batches of 61. The gradient synchronization itself weights each local gradient by its local sample count before dividing by the global count, so the tail batch was combined with the correct sample weighting and the comparison measures the effect of the batch size sequence rather than a weighting error.

The physical-MSE histories show that the unbalanced tail produced recurrent excursions that affected training and validation together. Figure~\ref{fig:04_final_trial_mse_comparison} plots the two histories on a common vertical scale from the \texttt{training\_physical\_mse} and \texttt{validation\_physical\_mse} columns of the two \texttt{epoch\_metrics.csv} files. Each curve is a single ordinary-training run and not a fold average, and validation was evaluated after the optimizer updates at every epoch. In \texttt{slurm-55367279\_trial\_1}, the reported spike at epoch 93 reached training MSE $0.0159286$ and validation MSE $0.0164889$, rising from $0.0049323$ and $0.0042915$ at epoch 92 and returning to $0.0047361$ and $0.0040539$ at epoch 94. Comparable spikes occurred at epochs 84, 102, and 151 with training values $0.0149974$, $0.0122572$, and $0.0113022$. After epoch 10, seven epochs exceeded $0.008$ on both training and validation in the first trial, namely epochs 18, 38, 68, 84, 93, 102, and 151. In the balanced second trial, no epoch after epoch 10 exceeded $0.008$ on either curve. The largest post-epoch-10 training MSE fell from $0.0159286$ at epoch 93 in the first trial to $0.0079512$ at epoch 18 in the second trial, a factor of $2.00$, while the corresponding validation maxima fell from $0.0164889$ to $0.0069026$, a factor of $2.39$. The value of approximately $0.00819$ sometimes quoted for the balanced run is the epoch-3 initial transient with training MSE $0.0081904$ and validation MSE $0.0079871$, not a late excursion. All 200 rows in both histories are finite, the effective learning rate is constant at $10^{-3}$ in every row, and the fact that training and validation move together at each spike is consistent with a noisy update affecting the model as a whole rather than with validation-only overfitting.

\begin{figure}[h!]
    \centering
    \includegraphics[width=0.95\linewidth]{Images/Chapter04/final_training/final_trial1_trial2_mse_comparison.png}
    \caption{Physical training and validation MSE for the two 200-epoch seed-42 runs at global batch size 64. The upper panel is \texttt{slurm-55367279\_trial\_1} with 24 batches of 64 plus one 6-sample tail and the lower panel is \texttt{slurm-55373639\_trial\_2} with 17 batches of 62 and 8 batches of 61. Both panels share the same linear vertical scale and show all 200 completed epochs. The annotation marks the epoch-93 excursion in the first trial. Both curves are single-run histories from \texttt{epoch\_metrics.csv}; the standardized SIMM objective is not shown.}
    \label{fig:04_final_trial_mse_comparison}
\end{figure}

The gradient and parameter-update diagnostics give a compatible signature of the same events without changing the selection of the runs. Figure~\ref{fig:04_final_trial_gradient_update} uses rows with \texttt{parameter\_scope=all} from \texttt{gradient\_norms.csv} and \texttt{parameter\_update\_ratios.csv}. The gradient diagnostic is measured after synchronized weighted MPI aggregation and before clipping and the optimizer update, and the update ratio is the actual relative movement in Equation~\eqref{eq:04_update_ratio}, which includes clipping, Adam moments, bias correction, and the current parameter scale. The left panel shows the RMS gradient norm on a logarithmic scale against the stable model layer index at epoch 93, where indices 0, 4, 6, and 8 are the convolution and the three dense layers. At that epoch the unbalanced trial recorded $1.1273$, $0.2701$, $0.4648$, and $0.3745$ across those layers, compared with $0.6212$, $0.1629$, $0.2658$, and $0.2269$ for the balanced control at the same epoch. The contrast is larger at epoch 85, where the corresponding first-layer values are $1.7509$ and $0.6289$. The right panel tracks the actual mean update ratio of the first dense layer, stable index 4, over all 200 epochs on a logarithmic scale. In the spike window the unbalanced trial reached $0.007360$ at epoch 85, $0.002348$ at epoch 93, and $0.003838$ at epoch 94, compared with $0.001825$, $0.001527$, and $0.001671$ for the balanced trial, with mean pre-update norms near $17.3$ to $17.6$ and zero near-zero denominator steps in all six rows, so the difference cannot be attributed to a small denominator. Two qualifications are needed. First, the global maxima over all epochs are dominated by the epoch-1 transient in both runs, with RMS norms $11.0317$ and $10.9581$ and update ratios $0.014199$ and $0.014015$, so the balanced run did not remove the initial transient but removed the repeated late spikes. Second, the activation statistics show no comparable spike at epoch 93 and are therefore not used as instability evidence here. The diagnostics thus support the MSE picture: finite values throughout, a constant learning rate, and elevated synchronized gradients and actual parameter movement during the excursions of the first trial.

\begin{figure}[h!]
    \centering
    \includegraphics[width=0.98\linewidth]{Images/Chapter04/final_training/final_trial1_trial2_gradient_update.png}
    \caption{Gradient and update signature of the epoch-93 excursion for the same two runs. The left panel shows the RMS gradient norm with \texttt{parameter\_scope=all} against stable model layer index at epoch 93 on a logarithmic scale. The right panel shows the actual mean update ratio with \texttt{parameter\_scope=all} for the first dense layer at stable index 4 over all 200 completed epochs on a logarithmic scale. Red is the unbalanced \texttt{slurm-55367279\_trial\_1} and green is the balanced \texttt{slurm-55373639\_trial\_2}. No bias-only rows are used.}
    \label{fig:04_final_trial_gradient_update}
\end{figure}

The numerical behaviour improved with balancing while the final accuracy for this pair did not. The restored checkpoints selected epoch 195 in the first trial with training MSE $0.0022016$, validation MSE $0.0018895$, and test MSE $0.0018235$, and epoch 190 in the second trial with $0.0024080$, $0.0019529$, and $0.0020304$, all read from the corresponding \texttt{final\_metrics.csv} files. The test set of 429 samples was loaded only after training and checkpoint restoration. The results indicate that removing the 6-sample tail eliminated the three-to-four-fold recurrent excursions and reduced the late gradient peaks, which is consistent with the interpretation that a much noisier gradient estimate from a handful of samples was periodically applied to the shared parameters even though the MPI averaging itself was sample-count correct. This remains an interpretation from two seed-42 runs rather than a proven causal mechanism, and balancing must be considered separately from accuracy: for this pair of runs, stability improved without improving the final test error.
