...
The recorded outcomes of this sequence are collected in Table~\ref{tab:04_final_training_trials}. These are eleven ordinary training runs with a single 90/10 holdout split of the 1713 training samples, not cross-validation folds. Normalization is fitted only on the 1542 training samples and applied unchanged to the 171 validation samples and the subsequent test evaluation. All runs use the topology \texttt{conv5x5-dense-1024-512-256-128}. All errors in the table are physical-unit MSE values: ``budget'' is the maximum configured epoch count, ``completed'' is the length of the diagnostic history, and ``selected'' identifies the restored validation-minimum checkpoint. The first four rows use seed 42, the following rows examine the stopping policy across seeds, and the last two rows increase the seed-42 budget.
...
\begin{table}[h!]
    ...
    \begin{tabularx}{\linewidth}{@{}Xrrrrrrr@{}}
        ...
        \texttt{trial 5}, seed 42 & 257 & 1200 & 860 & 797 & 0.000395 & 0.000435 & 0.000487 \\
        Final production, seed 42 & 257 & 2500 & 860 & 797 & 0.000395 & 0.000435 & 0.000487 \\
        \hline
    \end{tabularx}
    \caption{Ordinary training results from the eleven run directories under \texttt{results/ordinary-training}. Restored metrics come from \texttt{final\_metrics.csv}; budgets, batch construction and stopping policies come from metadata, and completed epochs from diagnostic histories. The final three seed-42 rows retain the same checkpoint and are not independent accuracy gains.}
    \label{tab:04_final_training_trials}
\end{table}

The first two trials compare batch construction while keeping the topology, split, initialization and optimizer settings fixed. Both \texttt{trial-1\_slurm-56688186} and \texttt{trial-2\_slurm-56688187} use seed 42, Adam at configured and effective learning rate $10^{-3}$, physics weight $0.10$, no dropout or L1 or L2 penalty, componentwise gradient clipping at 1, global batch size 64, and 64 MPI ranks. Both record the same source revision, \texttt{d9c1b4994e3c}, with the batch policy explicitly set to \texttt{range\_tail} or \texttt{balanced}. Their histories contain 25 optimizer steps per epoch, but the first run completes 200 epochs and 5000 updates whereas the second stops at epoch 191 after 4775 updates. Thus the configured update budgets agree, but the completed budgets do not. The tail is combined with correct sample weighting, so this comparison concerns the batch size sequence rather than an MPI weighting error.
...
The revised policy retains the $15\%$ ratio but uses a minimum of 20 epochs and a patience of 20 consecutive qualifying epochs. In \texttt{Trainer}, the streak resets before epoch 20, whenever validation reaches a new minimum, or whenever validation MSE is at most $1.15$ times training MSE. Otherwise it increases by one. A parameter snapshot is stored at every new validation minimum, and the best parameters are restored before the final metrics are recomputed. With this policy and the 834-epoch budget, seed 0 completes the budget and selects epoch 820, seed 1 stops at epoch 159 and selects epoch 139, and seed 42 completes the budget and selects epoch 797. Their validation MSE values are $0.0005521$, $0.0047519$ and $0.0004354$, respectively. The policy permits substantially longer optimization than the first-ratio rule, particularly for seeds 0 and 1, but the remaining variation shows that it does not remove sensitivity to initialization and the holdout split. The test values in Table~\ref{tab:04_final_training_trials} are post-training evaluations, not a criterion for ranking these seeds.

The final production configuration uses a $5\times5$ convolution from one input channel to eight output channels, with stride five and no padding. Flattening produces 7200 features; concatenating Reynolds number and angle of attack gives the 7202 inputs to the dense head $(1024,512,256,128,1)$. LeakyReLU with negative slope $0.05$ follows the convolution and each hidden dense layer, while the output is linear. This is the architecture constructed by the current \texttt{make\_single\_trial} in \texttt{CNN/main.cpp}. The recorded run, \texttt{production\_slurm-56688223}, belongs to the \texttt{re-tuning} experiment at source revision \texttt{d9c1b4994e3c}, where a separate driver generated these trials; its metadata matches the current production recipe, rather than establishing that the current executable produced the archived run. It uses Adam with learning rate $10^{-3}$, moment coefficients $0.9$ and $0.999$, $\epsilon=10^{-8}$, zero weight decay, physics weight $0.10$, no dropout or L1 or L2 penalty, componentwise gradient clipping at 1, global batch size 257, seed 42 and 64 MPI ranks. The maximum of 2500 epochs agrees with the explicit override in \texttt{training.sh}, whereas the default in \texttt{CNN/main.cpp} is 200 epochs.

Increasing the seed-42 budget from 834 to 1200 and then 2500 epochs does not produce three independent improvements. The 834-epoch run already selects epoch 797. Trial 5 and production both stop at epoch 860 and restore that same checkpoint, with identical diagnostic histories and final test artifacts between these two longer runs. Their history through epoch 834 also matches the seed-42 834-epoch run, and all three restore identical parameters. The extra budget allows the stopping rule to finish, but does not lower the retained validation minimum.
